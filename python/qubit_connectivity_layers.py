# qubit connectivity as a function of k-repetitions/layers of ansatz
...
OUT_CSV = Path(f"output/{molecule}_{basis}_dissociation_nreps.csv")
...
def energy_from_connectivity(mol_data, ccsd, label, n_reps: int):
    """Build UCJ op from CCSD t1/t2 using the requested connectivity and n_reps."""
    norb = mol_data.norb
...
def main():
    ...
    header = ["n_reps"] + list(CONNECTIVITIES.keys())
    ...
    for n_reps in range(1, 11):
        row = {"n_reps": str(n_reps)}
        for label in CONNECTIVITIES:
            e_val = energy_from_connectivity(mol_data, ccsd, label, n_reps)
